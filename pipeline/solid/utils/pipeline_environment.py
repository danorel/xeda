...
class PipelineEnvironment(gym.Env):
    def __init__(
        ...
        if self.target_set_id != None and target_items == None:
        ...
        self.set_state_dim = self.pipeline.discrete_categories_count * len(
            self.state_encoder.set_description
        )
        self.operation_state_dim = len(self.state_encoder.set_description)
        if self.mode == "by_example":
            self.set_state_dim += self.number_of_examples * len(
                self.pipeline.exploration_columns
            )
            self.operation_state_dim += self.number_of_examples * len(
                self.pipeline.exploration_columns
            )
    ...
    def reset(self, initial_sets=None):
        ...
        if self.mode == "concentrated":
            self.state_encoder = StateEncoder(
                self.pipeline,
                target_items=TargetSetGenerator.get_concentrated_target_set(),
            )
    ...
    def get_contentrated_start_datasets(self):
        ...
        return self.pipeline.by_facet(
            dataset,
            attributes=[column_to_split_on],
            number_of_groups=self.pipeline.discrete_categories_count,
        )
    ...
    def step(self, set_index, operation_index=-1):
        ...
        if set_action_array[0] == "by_superset":
            self.datasets = self.pipeline.by_superset(dataset=self.input_set)
        elif set_action_array[0] == "by_distribution":
            self.datasets = self.pipeline.by_distribution(dataset=self.input_set)
        elif set_action_array[0] == "by_facet":
            self.datasets = self.pipeline.by_facet(
                dataset=self.input_set,
                attributes=[set_action_array[1]],
                number_of_groups=10,
            )
        else:
            self.datasets = self.pipeline.by_neighbors(
                dataset=self.input_set, attributes=[set_action_array[1]]
            )

        self.datasets = [d for d in self.datasets if d.set_id != None and d.set_id >= 0]
        ...
        if len(self.datasets) == 0:
            ...
            ) = self.utility_manager.get_uniformity_scores(self.datasets, self.pipeline)
            ...
            (
                ...
                seen_sets,
                new_utility_weights,
            ) = self.utility_manager.get_novelty_scores_and_utility_weights(
                self.datasets,
                self.sets_viewed,
                self.pipeline,
            ...
            if self.utility_weights != None:
                ...
                if operation_identifier in self.previous_operations:
                    ...
                    self.utility = self.utility_manager.compute_utility(
            ...
            self.summary_evaluator.evaluate_sets(self.datasets)
        self.inverted_entropy_score = (
            self.summary_evaluator.get_inverted_entropy_score()
        )

        episode_info = {
            "input_set_index": set_index,
            "input_set_size": len(self.input_set.data),
            "input_set_id": self.input_set.set_id
            if self.input_set.set_id != None
            else -1,
            ...
            "output_set_average_size": sum(map(lambda x: len(x.data), self.datasets))
            / len(self.datasets),
            "reward": reward,
        ...
        set_op_id = (
            f"{self.input_set.set_id}:{set_action_array}"
            if self.input_set != None
            else f"-1:{set_action_array}"
        )
