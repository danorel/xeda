...
from constants.dataset import DATA_FOLDER
from .dataset import Dataset
from .predicate_item import JoinParameters, PredicateItem, PredicateItemGroup
...
class Pipeline:
    ...
    def __init__(
        self,
        database_name: str,
        initial_collection_names: List[str],
        ...
        self.initial_collection = self.load_table(self.initial_collection_names[0])
        if os.path.isfile(f"{self.database_folder}/foreign_keys.csv"):
            self.foreign_keys = pd.read_csv(f"{self.database_folder}/foreign_keys.csv")

        all_columns_to_discretize = pd.read_csv(
            f"{self.database_folder}/columns_to_discretize.csv"
        )
        self.columns_to_discretize = all_columns_to_discretize[
            all_columns_to_discretize.table.isin(initial_collection_names)
        ]
        if len(self.columns_to_discretize) > 0:
            self.columns_to_discretize = self.columns_to_discretize.apply(
                lambda column: f"{column.table}.{column.column}", axis=1
            ).to_list()
        ...
        for target_collection_name in tables_to_join:
            relations = self.foreign_keys[
                (
                    (self.foreign_keys["table1"].isin(joined_tables))
                    & (self.foreign_keys["table2"] == target_collection_name)
                )
                | (
                    (self.foreign_keys["table2"].isin(joined_tables))
                    & (self.foreign_keys["table1"] == target_collection_name)
                )
            ]
            if len(relations) == 1:
                ...
                if target_collection_name == relation.table1:
                    self.initial_joins.append(
                        JoinParameters(
                            target_collection_name=relation.table1,
                            left_attribute=f"{relation.table2}.{relation.attribute2}",
                            right_attribute=f"{relation.table1}.{relation.attribute1}",
                            other_collection=relation.table2,
                        )
                    )
                    self.initial_collection = pd.merge(
                        self.initial_collection,
                        target_collection,
                        how="inner",
                        left_on=f"{relation.table2}.{relation.attribute2}",
                        right_on=f"{relation.table1}.{relation.attribute1}",
                        sort=False,
                        suffixes=("", f"_{target_collection_name}"),
                    )
                else:
                    self.initial_joins.append(
                        JoinParameters(
                            target_collection_name=relation.table2,
                            left_attribute=f"{relation.table1}.{relation.attribute1}",
                            right_attribute=f"{relation.table2}.{relation.attribute2}",
                            other_collection=relation.table1,
                        )
                    )
                    self.initial_collection = pd.merge(
                        self.initial_collection,
                        target_collection,
                        how="inner",
                        left_on=f"{relation.table1}.{relation.attribute1}",
                        right_on=f"{relation.table2}.{relation.attribute2}",
                        sort=False,
                        suffixes=("", f"_{target_collection_name}"),
                    )
            elif len(relations) == 0:
                if (
                    tables_to_join.index(target_collection_name)
                    == len(tables_to_join) - 1
                ):
                    raise Exception("Unable to join " + target_collection_name)
        ...
        for column_name in self.columns_to_discretize:
            if column_name in self.initial_collection.columns.values:
                self.initial_collection[
                    column_name + "_original"
                ] = self.initial_collection[column_name]
                self.initial_collection[column_name] = pd.qcut(
                    self.initial_collection[column_name],
                    self.discrete_categories_count,
                    duplicates="drop",
                )
                self.ordered_dimensions[column_name] = list(
                    map(
                        lambda cat: str(cat),
                        self.initial_collection[column_name].dtype.categories,
                    )
                )

        if os.path.isfile(f"{self.database_folder}/columns_with_bins.json"):
            with open(
                f"{self.database_folder}/columns_with_bins.json"
            ) as columns_with_bins_file:
    ...
    def find_interesting_attributes(cls, dataframe, count=None):
        ...
        if count == None:
            interesting_attributes = attributes[
                (attributes.na_ratio < 0.3) & (attributes.density_ratio < 0.3)
            ].attribute.tolist()
    ...
    def load_table(self, table_name):
        forced_type_columns = self.column_forced_types[
            self.column_forced_types.table == table_name
        ]
        if len(forced_type_columns) > 0:
            ...
            return pd.read_csv(
                f"{self.database_folder}/{table_name}.csv", dtype={"rcn": "object"}
            ).add_prefix(table_name + ".")
        else:
            return pd.read_csv(f"{self.database_folder}/{table_name}.csv").add_prefix(
                table_name + "."
            )
        ...
        if return_datasets == False:
            ...
            for index, group in top_groups.iterrows():
                ...
                for i in range(len(attributes)):
                    if len(attributes) == 1:
                        new_dataset.predicate.append(
                            PredicateItem(
                                attributes[0],
                                "==",
                                index,
                                is_category=str(dataset.data[attributes[0]].dtype)
                                == "category",
                            )
                        )
                    else:
                        new_dataset.predicate.append(
                            PredicateItem(
                                ...
                                "==",
                                index[i],
                                is_category=str(dataset.data[attributes[i]].dtype)
                                == "category",
                            )
                        )
    ...
    def by_join(
        ...
        if drop_column == "both":
            dataset.data = dataset.data.drop(
                columns=[
                    join_parameters.left_attribute,
                    join_parameters.right_attribute,
                ]
            )
        if drop_column == "left":
            dataset.data = dataset.data.drop(columns=[join_parameters.left_attribute])
        if drop_column == "right":
            dataset.data = dataset.data.drop(columns=[join_parameters.right_attribute])
        return dataset
    ...
    def by_neighbors(self, dataset: Dataset, attributes: List[str], logger=None):
        ...
        if len(attributes) == 0:
            attributes = map(
                lambda item: item.attribute in self.ordered_dimensions,
                dataset.predicate.components,
            )
        ...
        for attribute in attributes:
            if attribute in self.ordered_dimensions:
                predicate_item = next(
                    (
                        item
                        for item in dataset.predicate.components
    ...
    def by_distribution(self, dataset: Dataset, logger=None):
        ordered_attributes_in_description = [
            x
            for x in dataset.predicate.get_attributes()
            if x in self.ordered_dimensions
        ]
                ...
                for i in range(len(ordered_attributes_in_description)):
                    ...
                    new_dataset.predicate.append(
                        PredicateItem(
                            attribute,
                            "==",
                            self.ordered_dimensions[attribute][vector[i]],
                            is_category=True,
                        )
                    )
                self.reload_set_data(
                    dataset=new_dataset, apply_joins=True, apply_predicate=True
                )
    ...
    def get_neighboring_set(self, dataset: Dataset, attribute: str, value_index: int):
        ...
        predicate_item = next(
            (
                item
                for item in new_dataset.predicate.components
                if item.attribute == attribute
            ),
            None,
        )
        predicate_item.value = self.ordered_dimensions[attribute][value_index]
        self.reload_set_data(new_dataset, apply_joins=True, apply_predicate=True)
        return new_dataset
        ...
        if apply_joins:
            for join_parameters in dataset.joins:
                ...
                dataset.data = pd.merge(
                    ...
                    right_on=join_parameters.right_attribute,
                    sort=False,
                    suffixes=("", f"_{join_parameters.target_collection_name}"),
                )
        else:
            dataset.joins = []
    ...
    def by_superset(
        self, dataset: Dataset, set_ids_to_ignore=[], number_of_sets_to_return=3
    ):
        if len(dataset.predicate.components) == 0:
            ...
            if len(predicate_attributes) == 1:
                ...
                candidates = sorted(
                    candidates, key=lambda candidate: len(candidate.data), reverse=True
                )
    ...
    def query_predicate(self, dataset: Dataset, predicate: PredicateItemGroup):
        for item in predicate.components:
            if item.is_multiple_line_string:
                ...
                index = self.ordered_dimensions[item.attribute].index(str(item.value))
                ...
                dataset.data = dataset.data[dataset.data[item.attribute] == item.value]
        return dataset.data
