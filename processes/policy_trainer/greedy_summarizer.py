...
class GreedySummarizer:
    ...
    def get_min_distance(self, groups):
        ...
        for index1, dataset1 in enumerate(groups):
            for dataset2 in groups[index1 + 1 : len(groups)]:
                distance = cityblock(dataset1["means_vector"], dataset2["means_vector"])
    ...
    def get_rax_min_uniformity(self, min_uniformity_target):
        for i in range(1024):
            if (
                self.utility_manager.zscore(
                    self.utility_manager.boxcox(
                        i, lmbda=self.utility_manager.uniformity_lambda
                    ),
                    self.utility_manager.uniformity_mean,
                    self.utility_manager.uniformity_std,
                )
    ...
    def get_summary(
        self, min_set_size=10, min_uniformity_target=2, result_set_count=10
    ):
        min_uniformity = self.get_rax_min_uniformity(min_uniformity_target)
        groups = self.pipeline.groups
        groups = (
            groups[
                (groups.member_count >= min_set_size)
                & (groups.uniformity >= min_uniformity)
            ]
            .sort_values(by="uniformity", ascending=False)
            .reset_index()
            .to_dict("records")
        )
        ...
        for group in tqdm(groups):
            if not group in selected_sets:
                ...
                for i in range(result_set_count):
                    ...
                    new_candidates_distance = self.get_min_distance(new_candidates)
