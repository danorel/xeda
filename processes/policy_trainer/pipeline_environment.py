...
from constants.dataset import TARGET_SET_PATH, TARGET_SET_KNOWN_PATH
...
class PipelineEnvironment(gym.Env):
    def __init__(
        ...
        target_set_df = pd.read_csv(TARGET_SET_PATH)
        if target_set_df.empty:
            raise ValueError(f"Target set should be generated by path: {str(TARGET_SET_PATH)}")
        target_items = json.loads(target_set_df.loc[target_set_df["id"] == target_set_id]["item_set"].values.tolist()[0])
