...
from .predicate_item import JoinParameters, PredicateItemGroup
...
class Dataset:
    def __init__(
        ...
        predicate: PredicateItemGroup = None,
        joins: List[JoinParameters] = None,
        tables=None,
        uniformity=None,
        means_vector=None,
        count=None,
        ...
        self.predicate: PredicateItemGroup = (
            predicate if predicate != None else PredicateItemGroup()
        )
    ...
    def get_count(self, sql_connection):
        self.count = pd.read_sql(
            self.get_sql_query(attributes=["count(*)"]), sql_connection
        ).iloc[0]["count"]
        return self.count
    ...
    def get_data(self, sql_connection, attributes=["*"], not_null_attribute=None):
        return pd.read_sql(
            self.get_sql_query(
                attributes=attributes, not_null_attribute=not_null_attribute
            ),
            sql_connection,
        )
    ...
    def get_sql_query(self, attributes=None, not_null_attribute=None):
        ...
        join_filters = list(
            map(
                lambda join: f"{join.left_attribute} = {join.right_attribute}",
                self.joins,
            )
        )
        if (
            len(self.predicate.components) > 0
            or len(join_filters) != 0
            or not_null_attribute != None
        ):
