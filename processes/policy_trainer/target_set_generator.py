...
class TargetSetGenerator:
    ...
    def get_diverse_target_set(number_of_samples=100):
        target_files = list(TARGET_SET_KNOWN_PATH.glob("*.json"))
        initial_target_set = []
        for target_file in target_files:
            with target_file.open('r') as f:
                target_set = json.load(f)
            if len(target_set) > number_of_samples:
                initial_target_set += random.choices(
                    target_set, k=number_of_samples
                )
            else:
    ...
    def get_concentrated_target_set():
        ...
        while True:
            ...
            with target_file.open('r') as f:
                target_set = json.load(f)
            if len(target_set) >= MIN_ITEM_SET_NODES and len(target_set) <= MAX_ITEM_SET_NODES:
