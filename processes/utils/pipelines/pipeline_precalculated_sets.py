...
from .predicate_item import PredicateItem
...
class PipelineWithPrecalculatedSets(Pipeline):
    def __init__(
        ...
        # )
        self.correspondences = pd.read_csv(f"{self.index_folder}/correspondences.csv")
    ...
    def get_neighboring_set(
        self, dataset: Dataset, attribute: str, value_index: int, return_data=True
    ):
        new_dataset = Dataset(
            predicate=copy.deepcopy(dataset.predicate),
            joins=copy.deepcopy(dataset.joins),
        ...
        if return_data:
            new_dataset = self.reload_set_data(
    ...
    def reload_set_data(
        self, dataset, apply_joins=False, apply_predicate=False, initialize_data=True
    ):
        dataset = Pipeline.reload_set_data(self, dataset, apply_joins, apply_predicate)
        try:
            self.set_dataset_precomputed_info(dataset)
        except IndexError:
            dataset.set_id = None
    ...
    def by_overlap(
        ...
        try:
            overlapping_groups_index = self.index.loc[dataset.set_id].sort_values(
                by="overlap", ascending=True
            )
            ...
            if len(overlapping_groups) <= number_of_groups:
                ...
                if len(interesting_indexes) == 0:
                    overlapping_groups.loc[
                        selected_groups_ids, "overlap_to_selection"
                    ] = 0
                else:
                    ...
                    for id in selected_groups_ids:
                        ...
                        overlap_to_selection = (
                            interesting_indexes.loc[
                                (id, other_selected_group_ids), "overlap"
                            ].sum()
                            if id in interesting_indexes.index
                            else 0
                        )
                        overlapping_groups.loc[
                            id, "overlap_to_selection"
                        ] = overlap_to_selection
                ...
                for (
                    unselected_group_id,
                    unselected_group,
                ) in overlapping_groups.iterrows():
                    if not unselected_group_id in selected_groups_ids:
                        if (
                            unselected_group.overlap > 0.1
                            or (datetime.now() - startTime).total_seconds()
                            > max_seconds
                            or selection_overlap_sum == 0
                        ):
                            break
                        sets_counter += 1
                        ...
                        for selected_group_id in selected_groups_ids:
                            ...
                            overlap_to_selection_if_replaced = (
                                indexes.loc[other_selected_group_ids, "overlap"].sum()
                                if unselected_group_id in indexes.index
                                else 0
                            )
                            overlapping_groups.loc[
                                unselected_group_id, "overlap_to_selection"
                            ] = overlap_to_selection_if_replaced
                            if (
                                overlap_to_selection_if_replaced
                                < overlapping_groups.loc[
                                    selected_group_id, "overlap_to_selection"
                                ]
                            ):
                                set_replacements_counter += 1
                                other_selected_group_ids.append(unselected_group_id)
                                ...
                                if len(interesting_indexes) == 0:
                                    ...
                                    for id in selected_groups_ids:
                                        if id != unselected_group_id:
                                            other_selected_group_ids = (
                                                selected_groups_ids.copy()
                                            )
                                            other_selected_group_ids.remove(id)
                                            overlap_to_selection = (
                                                interesting_indexes.loc[
                                                    (id, other_selected_group_ids),
                                                    "overlap",
                                                ].sum()
                                                if id in interesting_indexes.index
                                                else 0
            ...
            print("Overlap impossible: No set matching the current predicate was found")

    @loggable_operator()
    def by_distribution(
        self, dataset, return_datasets=True, logger=None, return_data=True
    ):
        ...
        for dataset in datasets:
            if len(dataset.data) > minimum_set_size:
                sets_with_info.append(
                    {
                        "dataset": dataset,
                        "dataset_sum": len(dataset.data[dataset.data.label == 1])
                        - len(dataset.data[dataset.data.label == 0]),
                        "dataset_size": len(dataset.data),
                    }
                )
        sets_with_info = sorted(
            sets_with_info,
            key=lambda i: (i["dataset_sum"], i["dataset_size"]),
            reverse=True,
        )
    ...
    @loggable_operator()
    def get_selectivity_list(
        self, dataset, attributes=None, ascending=False, logger=None
    ):
        ...
        for attribute in attributes:
            ...
            values.rename(
                columns={attribute: "selectivity", "index": "value"},
                inplace=True,
            )
    ...
    def get_groups_as_datasets(self, group_ids):
        ...
        for group_id in group_ids:
            ...
            for element_id in group.definition:
                element = self.correspondences.loc[
                    self.correspondences.id == element_id
                ].iloc[0]
                value = (
                    float(element.value)
                    if self.initial_collection[element.column].dtype == np.dtype(float)
                    else element.value
                )
                dataset.predicate.append(
                    PredicateItem(
                        ...
                        "==",
                        value,
                        is_category=str(self.initial_collection[element.column].dtype)
                        == "category",
                    )
                )
            datasets.append(
                self.reload_set_data(dataset, apply_joins=True, apply_predicate=True)
            )
        return datasets
    ...
    def set_dataset_precomputed_info(self, dataset: Dataset):
        try:
            ...
            groups = self.groups[self.groups.definition == group_definition]
            group_id = None
            if len(groups) == 1:
                ...
                if len(groups) > 1:
                    ...
                    same_length_groups = self.groups[
                        self.groups.member_count == len(dataset.data)
                    ]
                    groups = same_length_groups[
                        same_length_groups.definition.apply(
