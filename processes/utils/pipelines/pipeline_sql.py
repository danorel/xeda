...
class PipelineSql:
    ...
    def __init__(self, database_name: str, discrete_categories_count=20):
        ...
        self.sql_connection = psycopg2.connect(
            dbname=PipelineSql.databases[database_name],
            user="postgres",
        ...
        self.columns_to_discretize = all_columns_to_discretize.apply(
            ...
            if type(bins_data[column_name][0]) == list:
                if isinstance(bins_data[column_name][0][0], str):
                    for i in range(len(bins_data[column_name])):
                        bins_data[column_name][i] = list(
                            map(lambda x: pd.Timestamp(x), bins_data[column_name][i])
                        )
    ...
    def by_facet(
        ...
        for attribute in attributes:
            if attribute in self.interval_indexes:
                res = dataset.get_data(
                    self.sql_connection, [f"min({attribute})", f"max({attribute})"]
                )
                ...
                res = dataset.get_data(
                    self.sql_connection,
                    [f'distinct {attribute} as "{attribute}" '],
                    not_null_attribute=attribute,
                )
                ...
                for value in attributes_possible_values[attribute]:
                    ...
                    new_set.predicate.append(
                        PredicateItem(
                            ...
                            "==",
                            value,
                            is_category=type(value) == pd.Interval,
                        )
                    )
    ...
    def by_neighbors(self, dataset: Dataset, attributes: List[str], logger=None):
        if len(attributes) == 0:
            attributes = map(
                lambda item: item.attribute in self.interval_indexes,
        ...
        for attribute in attributes:
            if (
                attribute in self.interval_indexes
                or attribute in self.ordered_dimensions
            ):
                if attribute in self.interval_indexes:
                ...
                if predicate_item != None:
                    index = list(collection[attribute]).index(predicate_item.value)
                    if index != 0:
                        lower_set = dataset.copy()
                        predicate_item = next(
                            (
                                item
                                for item in lower_set.predicate.components
                                if item.attribute == attribute
                            ),
                            None,
                        )
                    ...
                    if index + 1 != len(collection[attribute]):
                        ...
                        predicate_item = next(
                            (
                                item
                                for item in higher_set.predicate.components
                                if item.attribute == attribute
                            ),
                            None,
                        )
    ...
    def by_superset(self, dataset: Dataset, set_ids_to_ignore=[]):
        if len(dataset.predicate.components) == 0:
            ...
            for i in range(len(dataset.predicate.components)):
                ...
                if (
                    smallest_item_count == None or new_count < smallest_item_count
                ) and new_count != original_count:
                    result = new_set
