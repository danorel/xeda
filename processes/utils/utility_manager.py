...
class UtilityManager:
    ...
    def normalize_uniformity(self, value):
        return self.zscore(
            self.boxcox(value, lmbda=self.uniformity_lambda),
            self.uniformity_mean,
            self.uniformity_std,
        )
    ...
    def get_min_distance(self, datasets, pipeline):
        ...
        if self.scale_values:
            return self.zscore(
                self.boxcox(min_distance, lmbda=self.diversity_lambda),
                self.diversity_mean,
                self.diversity_std,
            )
        else:
    ...
    def get_novelty_scores_and_utility_weights(
        self,
        datasets,
        seen_sets,
        pipeline,
        ...
        if self.scale_values:
            summary_score = self.zscore(
                self.boxcox(summary_score, lmbda=self.novelty_lambda),
                self.novelty_mean,
                self.novelty_std,
            )
        ...
        if decreasing_gamma:
            gamma = math.pow(
                (len(seen_sets | summary_set_ids) / (len(pipeline.groups) / 1000)) - 1,
                2,
            )
            if gamma < 0:
                gamma = 0
            ...
            alpha_beta = (
                math.pow(
                    (len(seen_sets | summary_set_ids) / (len(pipeline.groups) / 1000))
                    - 1,
                    2,
                )
                / 2
        ...
        return (
            summary_score,
            seen_sets | summary_set_ids,
            [alpha_beta, alpha_beta, gamma],
        )
    ...
    def get_future_scores(
        ...
        for index, dataset in enumerate(sets):
            if (
                dataset.set_id != None
                and dataset.set_id >= 0
                or len(dataset.data) == len(pipeline.initial_collection)
            ):
                ...
                for attribute in pipeline.exploration_columns:
                    if attribute in predicate_attributes:
                        ...
                        operation_identifier = (
                            f"{operation}-{attribute}-{dataset.set_id}"
                        )
                    ...
                    if not operation_identifier in previous_operations:
                        resulting_sets = [
                            d
                            for d in resulting_sets
                            if d.set_id != None and d.set_id >= 0
                        ]
                        if (
                            len(resulting_sets) > 0
                            and set(map(lambda x: x.set_id, resulting_sets))
                            != previous_dataset_ids
                        ):
                            (
                                summary_uniformity,
                                sets_uniformity_scores,
                            ) = self.get_uniformity_scores(resulting_sets, pipeline)
                            ...
                            (
                                ...
                                final_predicates,
                                new_utility_weights,
                            ) = self.get_novelty_scores_and_utility_weights(
                                resulting_sets,
                                seen_sets,
                                pipeline,
                            ...
                            operations.append(
                                {
                                    "setId": int(dataset.set_id)
                                    if dataset.set_id != None
                                    else None,
                                    "operation": operation,
                                    "attribute": attribute.replace(
                                        f"{pipeline.initial_collection_names[0]}.", ""
                                    ),
                ...
                if len(dataset.predicate.components) > 1:
                    ...
                    if not operation_identifier in previous_operations:
                        resulting_sets = pipeline.by_distribution(dataset=dataset)
                        resulting_sets = [
                            d
                            for d in resulting_sets
                            if d.set_id != None and d.set_id >= 0
