...
def push_pipeline_json(s3fs, bucket_name, pipeline_folder, pipeline_name, pipeline):
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        s3fs.put(f"{tempdir}/pipeline.json", f"{bucket_name}/{pipeline_folder}/{pipeline_name}.json")
...
def pull_pipeline_json(s3fs, bucket_name, pipeline_folder, pipeline_name):
    with tempfile.TemporaryDirectory() as tempdir:
        s3fs.get(f"{bucket_name}/{pipeline_folder}/{pipeline_name}.json", f"{tempdir}/pipeline.json")
