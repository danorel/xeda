    ...
    methods: {
        ...
        submitted() {
            if (this.sets.length !== 0 & this.selectedSetId === null) {
                ...
                const operatorUrl = new URL(this.webService + "/operators/" + this.operator + '-g')
                const explanationUrl = new URL(this.webService + "/explanation")
                ...
                if (!this.inputSet) {
                    operatorRequestData.input_set_id = -1
                    this.inputSet = null
                } else {
                    operatorRequestData.input_set_id = this.inputSet.id
                }
                ...
                operatorRequestData.weights_mode = this.guidanceWeightMode
                operatorRequestData.galaxy_class_scores = this.galaxy_class_scores
                this.predictedSetId = null
                this.predictedDimension = null

                axios.put(operatorUrl, operatorRequestData).then(response => {
                    if (this.inputSet)
